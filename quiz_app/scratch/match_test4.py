...
# Extract questions from PDFs with all options if possible
# Since we know options start with (A), (B), (C), (D)
pdf_data = []
...
for q in questions:
    ...
    opt0_json_full = clean_str(q['options'][0])
    
    best_match = None
    
    for pdf_q in pdf_data:
        pdf_q_clean = clean_str(pdf_q['q_text'])
        pdf_optA_clean = clean_str(pdf_q['optA'])
        
        # Method 1: match question end
        if q_end in pdf_q_clean and len(q_end) > 5:
            best_match = pdf_q['q_text']
            break
            
        # Method 2: match option A
        if opt0_json in pdf_optA_clean and len(opt0_json) > 5:
            best_match = pdf_q['q_text']
            break
            
        # Method 3: exact string match if short
        if opt0_json_full == pdf_optA_clean:
            best_match = pdf_q['q_text']
            break
